...
class WeeklyOverview
{
    ...
    {
        $nutrition = $this->db->query('SELECT food.name, food.fat, food.sugar, food.calorie, food.protein 
            FROM `user_food`
            INNER JOIN food
        ...
        $drinks = $this->db->query('SELECT drinks.name, drinks.fat, drinks.sugar, drinks.calorie, drinks.protine 
            FROM `user_drinks`
            INNER JOIN drinks
    ...
    {
        $counter = 0;
        $goals = $this->db->query('SELECT user_goals.task_quantity,user_goals.user_progress,goals.task
            FROM user_goals
            INNER JOIN goals
